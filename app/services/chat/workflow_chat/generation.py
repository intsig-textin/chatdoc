...
import logging
import traceback
from app.exceptions.http.chat import GenerationException
...
def generate_context(context: Context):
    ...
    """
    # 按照文件进行group
    max_length = settings.app.wf_chat.retrieval_max_length
    _context = ""

    file_uuid_mapper = {file.file_id: file for file in context.file_meta_list}
    if 1 <= len(context.file_meta_list) <= 10:
        each_length = max_length // len(context.file_meta_list)
    else:
